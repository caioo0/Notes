...
事件监控是 K8s 中比较另类的一种监控方式。之前的文章（[https://developer.aliyun.com/article/720438](https://link.zhihu.com/?target=https%3A//developer.aliyun.com/article/720438)）为大家介绍了在 K8s 中的一个设计理念，就是基于状态机的一个状态转换。从正常的状态转换成另一个正常的状态的时候，会发生一个 normal 的事件，而从一个正常状态转换成一个异常状态的时候，会发生一个 warning 的事件。通常情况下，warning 的事件是我们比较关心的，而事件监控就是可以把 normal 的事件或者是 warning 事件离线到一个数据中心，然后通过数据中心的分析以及报警，把相应的一些异常通过像钉钉或者是短信、邮件的方式进行暴露，弥补常规监控的一些缺陷和弊端。
...
![img](https://pic4.zhimg.com/80/v2-f4a752adc402932b545743d6aed36b93_1440w.webp)
...
![img](https://pic3.zhimg.com/80/v2-c43412f7ce17d95adc3e3d3cf0f8f392_1440w.webp)
...
![img](https://pic3.zhimg.com/80/v2-623f2fc2e6d2a2ade4e0e067de3405b6_1440w.webp)
...
对应的接口是 [http://metrics.k8s.io](https://link.zhihu.com/?target=http%3A//metrics.k8s.io)，主要的实现就是 metrics-server，它提供的是资源的监控，比较常见的是节点级别、pod 级别、namespace 级别、class 级别。这类的监控指标都可以通过 [http://metrics.k8s.io](https://link.zhihu.com/?target=http%3A//metrics.k8s.io) 这个接口获取到。

**第二类 Custom Metrics**

对应的 API 是 [http://custom.metrics.k8s.io](https://link.zhihu.com/?target=http%3A//custom.metrics.k8s.io)，主要的实现是 Prometheus。它提供的是资源监控和自定义监控，资源监控和上面的资源监控其实是有覆盖关系的，而这个自定义监控指的是：比如应用上面想暴露一个类似像在线人数，或者说调用后面的这个数据库的 MySQL 的慢查询。这些其实都是可以在应用层做自己的定义的，然后并通过标准的 Prometheus 的 client，暴露出相应的 metrics，然后再被 Prometheus 进行采集。

而这类的接口一旦采集上来也是可以通过类似像 [http://custom.metrics.k8s.io](https://link.zhihu.com/?target=http%3A//custom.metrics.k8s.io) 这样一个接口的标准来进行数据消费的，也就是说现在如果以这种方式接入的 Prometheus，那你就可以通过 [http://custom.metrics.k8s.io](https://link.zhihu.com/?target=http%3A//custom.metrics.k8s.io) 这个接口来进行 HPA，进行数据消费。
...
那怎么去消费呢？也是在 K8s 里面实现了一个标准，就是 [http://external.metrics.k8s.io](https://link.zhihu.com/?target=http%3A//external.metrics.k8s.io)。主要的实现厂商就是各个云厂商的 provider，通过这个 provider 可以通过云资源的监控指标。在阿里云上面也实现了阿里巴巴 cloud metrics adapter 用来提供这个标准的 [http://external.metrics.k8s.io](https://link.zhihu.com/?target=http%3A//external.metrics.k8s.io) 的一个实现。
...
![img](https://pic4.zhimg.com/80/v2-09a6cecab21fbe2ab133d86e74486d23_1440w.webp)
...
![img](https://pic4.zhimg.com/80/v2-357abb6641ce89827be7fd475fcd7567_1440w.webp)
