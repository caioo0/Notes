...
1. 相同配置下进行数据查询，Spark SQL 和 Hive on spark理论上和实际上有性能差距吗？

解答： 商业版更多使用 hive on tez 或者 impala， Spark SQL 和 Hive on spark底层是一样的，应该在性能上差不多。

2. Map任务处理block数据块的时候并行度是否可以控制？

结论：map任务的并行度无法设置，map任务的并行度跟数据块数量一致。如果希望减少map task的数量，可以通过设置数据块大小（比如上传时设置一个数据块大小为256M）

3. 通常来说，Spark与MapReduce相比，Spark运行效率更高。请说明效率更高来源于Spark内置的哪些机制？

spark借鉴了MapReduce，并在其基础上发展起来的，继承了其分布式计算的优点并进行了改进，spark生态更为丰富，功能更为强大，性能更加适用范围广，mapreduce更简单，稳定性好。主要区别：
...
1. https://zhuanlan.zhihu.com/p/88956461